...
function UserList() {
    ...
    useEffect(() => {
        setLoading(true)
        axios.get(`https://jsonplaceholder.typicode.com/users/${count}`).then((res) => {
            setLoading(false)
    ...
    return (
        <div classNameName='flex justify-around'>
            <div className="max-w-sm bg-white shadow-lg rounded-lg overflow-hidden my-4">
                <img className="w-full h-56 object-cover object-center" src="https://w1.pngwing.com/pngs/743/500/png-transparent-circle-silhouette-logo-user-user-profile-green-facial-expression-nose-cartoon.png" alt="avatar" />
                {loading && (
                    <div className="flex items-center px-6 py-3 bg-gray-900">
                        <h1 className="px-2 text-sm">Loading...</h1>
                    </div>
                )}
                {userData && (
                    <>
                        <div className="flex items-center px-6 py-3 bg-gray-900">
                            <h1 className="px-2 text-sm">{userData.company.name}</h1>
                        </div>
                        <div className="py-4 px-6">
                            <h1 className="text-2xl font-semibold text-gray-800">{userData.name}</h1>
                            <p className="py-2 text-lg text-gray-700">{userData.company.bs}, {userData.company.catchPhrase}</p>
                ...
                {error && (
                    <p classNameName='text-red-500'>Somthing wrong</p>
                )}
            </div>
            <div classNameName='flex justify-center align-middle'>
                <div classNameName='mt-8 h-[100px]'>
                    <div classNameName='flex justify-center'>
                        <div classNameName='w-[150px] h-[100px] rounded-lg mb-4 bg-slate-400 flex justify-center align-middle'>
                            <span classNameName='text-[50px]'>{count}</span>
                        </div>
                    </div>
